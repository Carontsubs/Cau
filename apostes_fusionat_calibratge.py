...
# Cicle únic fusionat (mitjana Primitiva 297d + Bonoloto 251d)
CICLE_FUSIONAT = 274
...
def carregar_sorteigs_fusionats():
    """Carrega tots els sorteigs de les dues loteries fusionats i ordenats per data"""
    sorteigs = []
    for fitxer in [FITXER_PRIMI, FITXER_BONO]:
...
def detectar_ultim_calibratge(sorteigs, cicle, finestra=90):
    """Detecta l'últim calibratge i estima el proper usant cicle fix"""
    resultats = []
...
def generar_recomanacio(config, ultim_calibratge, proper_calibratge):
    ...
    print(f"\n{'═'*60}")
    print(f"🎫 TIQUET FUSIONAT - {avui.strftime('%d/%m/%Y')}")
    print(f"   {config['descripcio']}")
    print(f"   Cicle únic: {CICLE_FUSIONAT}d (mitjana Primitiva+Bonoloto)")
...
def guardar_i_enviar_apostes(avui, aposta_a, aposta_b, mode, proper_calibratge):
    ...
    tiquet = (
        f"🎫 *NOU TIQUET* - {avui.strftime('%d/%m/%Y')} {mode}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🎰 *APOSTA PRINCIPAL*\n"
        ...
        f"💡 _Cicle únic fusionat {CICLE_FUSIONAT}d_"
    )
...
if __name__ == "__main__":
    ...
    # Carrega tots els sorteigs fusionats per detectar calibratge
    sorteigs_fusionats = carregar_sorteigs_fusionats()

    # Detecta l'últim calibratge amb cicle fix de 274 dies
    ultim_calibratge, proper_calibratge = detectar_ultim_calibratge(sorteigs_fusionats, CICLE_FUSIONAT)

    if not ultim_calibratge:
        print("❌ No s'ha pogut detectar el calibratge.")
        exit()

    # Determina el mode
    mode, dies_desde, dies_fins = determinar_mode(ultim_calibratge, proper_calibratge)

    print(f"\n📅 ESTAT DEL CICLE (fusionat {CICLE_FUSIONAT}d):")
    print(f"   Mode: {mode} | {dies_desde}d des del calibratge | {dies_fins}d fins al proper")
    print(f"   Últim calibratge: {ultim_calibratge.strftime('%d/%m/%Y')}")
    print(f"   Proper calibratge: {proper_calibratge.strftime('%d/%m/%Y')}")
