...
# Cicles reals detectats per separat
CICLE_PRIMI = 297
CICLE_BONO = 251
CICLE_FUSIONAT = (CICLE_PRIMI + CICLE_BONO) // 2  # 274 dies
...
def carregar_sorteigs(fitxer):
    """Carrega els sorteigs d'un CSV ordenats per data"""
    sorteigs = []
...
def calcular_calibratge_hibrid(sorteigs_primi, sorteigs_bono):
    """Detecta calibratge per separat i calcula la mitjana de dates"""
    ...
    print(f"\n🔬 DETECCIÓ DE CALIBRATGE:")
    print(f"   🔵 Primitiva: últim {ultim_primi.strftime('%d/%m/%Y')} → proper {proper_primi.strftime('%d/%m/%Y')}")
    print(f"   🟡 Bonoloto:  últim {ultim_bono.strftime('%d/%m/%Y')} → proper {proper_bono.strftime('%d/%m/%Y')}")

    # Mitjana de les dues dates detectades
    ultim_mitjana = ultim_primi + (ultim_bono - ultim_primi) / 2
    proper_mitjana = ultim_mitjana + timedelta(days=CICLE_FUSIONAT)

    print(f"   ⚖️  Mitjana:   últim {ultim_mitjana.strftime('%d/%m/%Y')} → proper {proper_mitjana.strftime('%d/%m/%Y')}")

    return ultim_mitjana, proper_mitjana
...
def configurar_finestres(mode):
    ...
    if mode == "POST":
        ...
        return {
            'dies_biaix': CICLE_FUSIONAT,
            'dies_momentum': max(25, CICLE_FUSIONAT // 10),
            'descripcio': '🟢 MADURESA (finestra completa, alta fiabilitat)',
            'emoji': '🟢'
        }


def generar_recomanacio(config, ultim_calibratge, proper_calibratge):
    """Analitza les dades fusionades i genera les apostes"""
    avui = datetime.now()
    lim_mom = avui - timedelta(days=config['dies_momentum'])
    lim_biaix = ultim_calibratge
    ...
    if not os.path.exists(FITXER_NETA):
        print("❌ No s'ha trobat la base de dades.")
        return None, None
    ...
    print(f"\n{'═'*60}")
    print(f"🎫 TIQUET HÍBRID - {avui.strftime('%d/%m/%Y')}")
    print(f"   {config['descripcio']}")
    print(f"   Finestra biaix: des de {ultim_calibratge.strftime('%d/%m/%Y')} ({(avui - ultim_calibratge).days}d)")    
    print(f"   Sorteigs analitzats: {sort_biaix} (biaix) / {sort_mom} (momentum)")
    print(f"   Últim calibratge (mitjana): {ultim_calibratge.strftime('%d/%m/%Y')}")
    print(f"   Proper calibratge estimat: {proper_calibratge.strftime('%d/%m/%Y')} ({(proper_calibratge - avui).days} dies)")
...
def comprovar_apostes():
    if not os.path.exists('apostes_actuals_fusionades.csv'):
        print("⚠️  No hi ha apostes anteriors guardades.")
        return

    apostes = {}
    data_generacio = ""
    with open('apostes_actuals_fusionades.csv', mode='r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for fila in reader:
    ...
    etiquetes = {'Principal': '🎰 PRINCIPAL', 'Secundaria': '🎰 SECUNDÀRIA'}
...
def guardar_i_enviar_apostes(avui, aposta_a, aposta_b, mode, proper_calibratge):
    with open('apostes_actuals_fusionades.csv', 'w', encoding='utf-8', newline='') as f:
    ...
    print("\n💾 Apostes guardades a apostes_actuals_fusionades.csv")

    dies_calibratge = (proper_calibratge - avui).days
    tiquet = (
        f"🎫 *NOU TIQUET HÍBRID* - {avui.strftime('%d/%m/%Y')} {mode}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🎰 *APOSTA PRINCIPAL*\n"
        ...
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"⏳ Proper calibratge: {proper_calibratge.strftime('%d/%m/%Y')} ({dies_calibratge}d)\n"
        f"💡 _Detecció separada, apostes fusionades_"
    )
...
if __name__ == "__main__":
    ...
    # Detecta calibratge per separat (més fiable)
    sorteigs_primi = carregar_sorteigs(FITXER_PRIMI)
    sorteigs_bono = carregar_sorteigs(FITXER_BONO)

    # Calcula la data de calibratge híbrida (mitjana de les dues deteccions)
    ultim_calibratge, proper_calibratge = calcular_calibratge_hibrid(sorteigs_primi, sorteigs_bono)

    # Determina el mode
    mode, dies_desde, dies_fins = determinar_mode(ultim_calibratge, proper_calibratge)

    print(f"\n📅 ESTAT DEL CICLE HÍBRID ({CICLE_FUSIONAT}d):")
    print(f"   Mode: {mode} | {dies_desde}d des del calibratge | {dies_fins}d fins al proper")

    # Configura finestres segons el mode
    config = configurar_finestres(mode)

    # Genera les apostes amb dades fusionades
    aposta_a, aposta_b = generar_recomanacio(config, ultim_calibratge, proper_calibratge)

    if aposta_a:
        guardar_i_enviar_apostes(avui, aposta_a, aposta_b, config['emoji'], proper_calibratge)
