...
FINESTRA_CONGELADES = 10  # sorteigs recents per detectar si una bola ha despertat
...
def calcular_retard_actual(sorteigs_cicle):
    """Calcula els sorteigs que porta cada bola sense sortir"""
    ultim_vist = {n: -1 for n in range(1, 50)}
    for idx, (_, nums) in enumerate(sorteigs_cicle):
        for n in nums:
            ultim_vist[n] = idx
    total = len(sorteigs_cicle)
    return {n: (total - ultim_vist[n] - 1) if ultim_vist[n] != -1 else total
            for n in range(1, 50)}


def generar_secundaria_congelades(sorteigs_cicle, stats_hist, freq_esperada):
    """
    Genera l'aposta secundària de la Primitiva amb boles CONGELADES:
    - Fredes (<75% de freq esperada)
    - Sense aparicions als últims FINESTRA_CONGELADES sorteigs (congelades pures)
    - Ordenades per retard descendent
    """
    fredes = [n for n in range(1, 50) if stats_hist[n] < freq_esperada * 0.75]

    ultims = sorteigs_cicle[-FINESTRA_CONGELADES:] if len(sorteigs_cicle) >= FINESTRA_CONGELADES else sorteigs_cicle
    freq_recent = Counter()
    for _, nums in ultims:
        for n in nums:
            freq_recent[n] += 1

    congelades = [n for n in fredes if freq_recent[n] == 0]
    retard = calcular_retard_actual(sorteigs_cicle)
    congelades.sort(key=lambda x: retard[x], reverse=True)

    print(f"\n   ❄️  Boles CONGELADES disponibles ({len(congelades)}): {congelades}")

    if len(congelades) < 6:
        despertant = [n for n in fredes if n not in congelades]
        despertant.sort(key=lambda x: freq_recent[x])
        congelades += despertant
        print(f"   ⚠️  Poques congelades, completant amb fredes despertant")

    return sorted(congelades[:6])
...
def analitzar_loteria(fitxer, nom_loteria, cicle, dies_momentum, ultim_calibratge, proper_calibratge):
    ...
    for n in range(1, 50):
        ...
        if stats_mom[n] > (sort_mom * 0.15):
            punts_mom = 40
            punts += 40
        elif significatius[n]['desviacio'] > 0 and significatius[n]['p'] < 0.05:
            punts_mom = 20
            punts += 20        
        if stats_hist[n] > 0:
    ...
    if nom_loteria == "Primitiva":
        aposta_b = generar_secundaria_congelades(sorteigs_cicle, stats_hist, freq_esperada)
        etiqueta_b = "❄️  CONGELADES"
    else:
    ...
    print(f"🎰 APOSTA PRINCIPAL:  👉 {' - '.join(f'{n:02d}' for n in aposta_a)}  🔥 TRIPLETS")
    print(f"🎰 APOSTA SECUNDÀRIA: 👉 {' - '.join(f'{n:02d}' for n in aposta_b)}  {etiqueta_b}")
    print(f"{'═'*60}")
...
def comprovar_apostes():
    ...
    etiquetes = {
        ...
        'Primi_Secundaria': '🔵 PRIMITIVA SECUNDÀRIA ❄️',
...
def guardar_i_enviar_apostes(avui, primi_a, primi_b, bono_a, bono_b,
    ...
    tiquet = (
        ...
        f"❄️  Secundària: `{' - '.join(f'{n:02d}' for n in primi_b)}`\n"
